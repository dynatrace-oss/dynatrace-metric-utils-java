...
class UnitValidatorTest {
  private static Stream<Arguments> provideValidUnits() {
    Stream<Arguments> lowercaseLetters = IntStream.rangeClosed(CodePoints.A_LOWERCASE, CodePoints.Z_LOWERCASE)
      .mapToObj(cp -> Arguments.of(String.format("valid lowercase %s", TestUtils.codePointToString(cp)), TestUtils.codePointToString(cp)));

    Stream<Arguments> uppercaseLetters = IntStream.rangeClosed(CodePoints.A_UPPERCASE, CodePoints.Z_UPPERCASE)
      .mapToObj(cp -> Arguments.of(String.format("valid uppercase %s", TestUtils.codePointToString(cp)), TestUtils.codePointToString(cp)));

    Stream<Arguments> digits = IntStream.rangeClosed(0, 9).mapToObj(i -> Arguments.of(String.format("valid digit %d", i), String.valueOf(i)));
    ...
    return Stream.concat(lowercaseLetters, Stream.concat(uppercaseLetters, Stream.concat(digits, specialCases)));
  }
  ...
  public static Stream<Arguments> provideInvalidUnits() {
    return Stream.of(
      ...
      // these are all characters that might occur in the metric line itself but not in the unit string.
